...
std::string SQLAbfrage::to_string(std::vector<std::vector<char>>& Spielfeld, std::string& finaler_string, int zeile, int spalte) 
{// soll das Spielfeld von einem 2-Dimensionalen Vektor in einen std::string nach Schema "fffufffufff" umwandeln. Die Zeitkomplexität ist O(n²).   
...
std::vector<std::vector<char>> SQLAbfrage::to_vector(std::string& Spielfeld, std::vector<std::vector<char>>& finaler_vector, size_t zeile)
{
    if(Spielfeld.empty())
    {
...
std::vector<std::vector<char>> SQLAbfrage::getSpielfeld(int ID)
{
...
void SQLAbfrage::setSpielfeld(int ID, std::vector<std::vector<char>> Spielfeld)
{
