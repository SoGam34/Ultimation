...
{
    if(x_pos == 0){return tictactoe_field[x_pos][y_pos];} // returns if no more left fields
    if(tictactoe_field[x_pos][y_pos] == tictactoe_field[x_pos-1][y_pos]){ return checkleft(x_pos-1,y_pos);} // goes left recursively
    return NULL;
}

mark Tictactoe_field::checkright(std::size_t x_pos , std::size_t y_pos)
{
    if(x_pos == tictactoe_field.size()-1){return tictactoe_field[x_pos][y_pos];} // returns value of field if no more left fields
    if(tictactoe_field[x_pos][y_pos] == tictactoe_field[x_pos+1][y_pos]){ return checkright(x_pos+1,y_pos);} // goes recursively to the right
    return NULL;
}

mark Tictactoe_field::checkup(std::size_t x_pos , std::size_t y_pos)
{
    if(y_pos == 0){return tictactoe_field[x_pos][y_pos];} // returns value of field if no more upper fields
    if(tictactoe_field[x_pos][y_pos] == tictactoe_field[x_pos][y_pos-1]){ return checkright(x_pos,y_pos-1);} // goes recursively up
    return NULL;
}

mark Tictactoe_field::checkdown(std::size_t x_pos , std::size_t y_pos)
{
    if(y_pos == tictactoe_field.size()-1){return tictactoe_field[x_pos][y_pos];} // returns value of field if no more lower fields
    if(tictactoe_field[x_pos][y_pos] == tictactoe_field[x_pos][y_pos+1]){ return checkright(x_pos,y_pos+1);} // goes recursively down
    return NULL;
}

mark Tictactoe_field::checkleftup(std::size_t x_pos , std::size_t y_pos)
{
    if(y_pos == 0 && x_pos == 0){return tictactoe_field[x_pos][y_pos];}
    if(tictactoe_field[x_pos][y_pos] == tictactoe_field[x_pos-1][y_pos-1]){ return checkright(x_pos-1,y_pos-1);}
    return NULL;
}mark Tictactoe_field::checkleftdown(std::size_t x_pos , std::size_t y_pos)
{
    if(y_pos == tictactoe_field.size()-1 && x_pos == 0){return tictactoe_field[x_pos][y_pos];}
    if(tictactoe_field[x_pos][y_pos] == tictactoe_field[x_pos-1][y_pos+1]){ return checkright(x_pos-1,y_pos+1);}
    return NULL;
}mark Tictactoe_field::checkrightup(std::size_t x_pos , std::size_t y_pos)
{
    if(y_pos == 0 && x_pos == tictactoe_field.size()-1){return tictactoe_field[x_pos][y_pos];}
    if(tictactoe_field[x_pos][y_pos] == tictactoe_field[x_pos+1][y_pos-1]){ return checkright(x_pos+1,y_pos-1);}
    return NULL;
}mark Tictactoe_field::checkrightdown(std::size_t x_pos , std::size_t y_pos)
{
    if(y_pos == tictactoe_field.size()-1 && x_pos == tictactoe_field.size()-1){return tictactoe_field[x_pos][y_pos];}
    if(tictactoe_field[x_pos][y_pos] == tictactoe_field[x_pos+1][y_pos+1]){ return checkright(x_pos+1,y_pos+1);}
    return NULL;
}
